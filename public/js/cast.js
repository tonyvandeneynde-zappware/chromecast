...
com.zappware.chromecast.cast.init = function(playbackConfig) {
    ...
    playerManager.setMessageInterceptor(cast.framework.messages.MessageType.PAUSE, function (data) {
        ...
        if (_.includes(com.zappware.chromecast.cast._externalRequests, data.requestId)) {
            userInitiated = true
        }
    ...
    function _addExternalRequest(event) {
        if (!com.zappware.chromecast.cast._externalRequests) {
            com.zappware.chromecast.cast._externalRequests = [];
        }
        const requestId = event.requestData && event.requestData.hasOwnProperty('requestId') && event.requestData.requestId;
        if (!_.includes(com.zappware.chromecast.cast._externalRequests, requestId)) {
            com.zappware.chromecast.cast._externalRequests.unshift(requestId);

            // Keep only 50
            if (com.zappware.chromecast.cast._externalRequests.length > 50) {
